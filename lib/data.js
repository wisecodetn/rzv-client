...
/** Lightweight establishment suggestions for the search autocomplete. Only
 *  called when the user has typed >3 chars — returns a few light rows, not the
 *  whole salon list. */
export async function suggestSalons(q, limit = 6) {
  const term = (q || "").trim()
  if (term.length <= 3) return []
  const light = (s) => ({ slug: s.slug, name: s.name, kind: s.kind, city: s.city })
  try {
    const r = await apiGet(`/public/salons?q=${encodeURIComponent(term)}`)
    return r.mapItems.slice(0, limit).map(light)
  } catch {
    return MOCK.searchSalons({ q: term }).items.slice(0, limit).map(light)
  }
}
