...
export default async function OpengraphImage({ params }) {
  ...
  return new ImageResponse(
    (
      <div style={{ width: "100%", height: "100%", display: "flex", position: "relative", background: "linear-gradient(135deg,#3A2B1A,#6B4E2E 55%,#2A1F12)" }}>
        {bg && (
        ...
        <div style={{ position: "absolute", top: 0, left: 0, width: "100%", height: "100%", display: "flex", flexDirection: "column", justifyContent: "center", padding: "80px 84px" }}>
          <div style={{ display: "flex", fontSize: 26, color: "#E2BB88", letterSpacing: 6, fontWeight: 700 }}>REZERVY</div>
          <div style={{ display: "flex", fontSize: 68, fontWeight: 700, color: "#FDF8EF", marginTop: 22, maxWidth: 940, lineHeight: 1.08 }}>{title}</div>
          <div style={{ display: "flex", fontSize: 30, color: "#D9BE97", marginTop: 24 }}>
