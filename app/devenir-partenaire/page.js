...
export default function PartnerPage() {
  return (
    <>
      {/* Hero */}
      <section style={{ background: "linear-gradient(120deg,#241A10,#4A3620 58%,#2A1F12)", color: "#FDF8EF" }}>
        <div className="wrap" style={{ padding: "72px 24px 66px" }}>
          ...
          <div style={{ display: "flex", gap: 10, marginTop: 26, flexWrap: "wrap" }}>
            <a href={PRO_URL} target="_blank" rel="noopener noreferrer" style={{ background: "#E2BB88", color: "#2A1A08", border: "none", borderRadius: 12, padding: "14px 26px", fontWeight: 800, fontSize: 14 }}>Devenir partenaire</a>
            <a href={PRO_URL} target="_blank" rel="noopener noreferrer" style={{ background: "rgba(253,248,239,0.1)", color: "#FDF8EF", border: "1px solid rgba(253,248,239,0.3)", borderRadius: 12, padding: "14px 24px", fontWeight: 700, fontSize: 14 }}>Voir une démo</a>
          </div>
          <div style={{ display: "flex", gap: 34, marginTop: 40, flexWrap: "wrap" }}>
            {STATS.map((s) => (
              <div key={s.l}>
                <div className="serif" style={{ fontSize: 30, color: "#E2BB88" }}>{s.v}</div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(280px,1fr))", gap: 16, marginTop: 30 }}>
          {BENEFITS.map((b) => (
            <div key={b.t} style={{ background: "var(--card)", border: "1px solid var(--line)", borderRadius: 18, padding: "22px 22px 24px" }}>
              <div style={{ width: 44, height: 44, borderRadius: 12, background: "rgba(169,124,72,0.12)", display: "flex", alignItems: "center", justifyContent: "center" }}><Icon d={b.d2} /></div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(240px,1fr))", gap: 18, marginTop: 26 }}>
            {[["1", "Créez votre demande", "Renseignez votre établissement et vos documents — validation sous 48h."], ["2", "Configurez votre salon", "Prestations, équipe, horaires et moyens de paiement en quelques minutes."], ["3", "Recevez vos réservations", "Votre page publique est en ligne : les clientes réservent, vous êtes notifié·e."]].map(([i, t, d]) => (
              <div key={i} style={{ textAlign: "center", padding: "0 12px" }}>
                <div style={{ width: 46, height: 46, borderRadius: "50%", background: "rgba(169,124,72,0.12)", color: "var(--gold-dark)", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 800, fontSize: 16, margin: "0 auto" }}>{i}</div>
      ...
      <section className="wrap" style={{ padding: "48px 24px 64px" }}>
        <div style={{ background: "linear-gradient(140deg,#3A2B1A,#6B4E2E)", borderRadius: 22, padding: "40px 34px", textAlign: "center", color: "#F8F0E2" }}>
          <div style={{ fontSize: 12.5, fontWeight: 800, letterSpacing: "0.06em", textTransform: "uppercase", color: "#D9BE97" }}>Sans engagement</div>
          <div className="serif" style={{ fontSize: 34, marginTop: 10 }}>Dès 49 TND / mois</div>
          <div style={{ fontSize: 14, color: "#D9BE97", marginTop: 8, maxWidth: 520, marginLeft: "auto", marginRight: "auto", lineHeight: 1.6 }}>
          ...
          <div style={{ display: "flex", gap: 10, justifyContent: "center", marginTop: 24, flexWrap: "wrap" }}>
            <a href={PRO_URL} target="_blank" rel="noopener noreferrer" style={{ background: "#E2BB88", color: "#2A1A08", border: "none", borderRadius: 12, padding: "14px 28px", fontWeight: 800, fontSize: 14 }}>Commencer gratuitement</a>
          </div>
        </div>
      </section>
    </>
